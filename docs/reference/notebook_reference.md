...
def load_frd_spot(ticker, directory):
    ...
    try:
        df = pd.read_csv(file_path, names=frd_columns, parse_dates=["datetime"])
...
def build_dollar_bars(
    spot_df: pd.DataFrame, asset_label: str, quantile: float = 0.80
) -> tuple:
...
try:
    ...
    print(
        "❌ Error: 'btc_spot_hf' not defined. Please verify you ran the Data Loading cell above."
    )
```
...
def plot_dollar_bar_sampling(
    raw_df, dollar_df, asset_name, start_date="2022-09-01", end_date="2022-09-30"
):
...
plot_dollar_bar_sampling(
    eth_spot_hf, eth_dollar_bars, "ETH", start_date="2022-09-10", end_date="2022-09-20"
)
```
...
def create_continuous_futures(all_contracts_df, roll_days_before_expiry=14):
    ...
    for i, contract in enumerate(contracts):
        ...
        if i < len(contracts) - 1:
            next_contract_data = df[df["expiry"] == contracts[i + 1]].sort_values(
                "datetime"
            )
        ...
        if i == len(contracts) - 1:
            ...
            if next_contract in adjusted:
                ...
                next_close_slice = next_segment.loc[
                    next_segment["datetime"] > roll_time, "adjusted_price"
                ]
            else:
            ...
            if not current_close_slice.empty and not next_close_slice.empty:
                ...
                next_price = next_close_slice.iloc[
                    0
                ]  # This previously crashed if empty
                adj = next_price - current_price
            else:
                # Fallback if gap is too large: assume 0 adjustment to prevent crash
                adj = 0
                print(
                    f"⚠️ Warning: Gap detected at roll for {contract}. Adjustment set to 0."
                )
            # --- BUG FIX END ---
    ...
    for contract in contracts:
        ...
        if not seg.empty:
            pieces.append(
                seg[["datetime", "close", "adjusted_price", "adjustment", "contract"]]
            )
...
print(
    f"  BTC: {len(btc_continuous):,} obs | {btc_continuous['datetime'].min().date()} to {btc_continuous['datetime'].max().date()}"
)
print(
    f"  ETH: {len(eth_continuous):,} obs | {eth_continuous['datetime'].min().date()} to {eth_continuous['datetime'].max().date()}"
)
```
...
btc_plot_copy["contract_changed"] = btc_plot_copy["contract"] != btc_plot_copy[
    "contract"
].shift(1)
...
eth_plot_copy["contract_changed"] = eth_plot_copy["contract"] != eth_plot_copy[
    "contract"
].shift(1)
...
ax3.set_title(
    "BTC: Cumulative Backward Adjustment Amount", fontsize=16, fontweight="bold", pad=15
)
...
ax4.set_title(
    "ETH: Cumulative Backward Adjustment Amount", fontsize=16, fontweight="bold", pad=15
)
...
def calculate_rolling_rv_vectorized(hf_data, target_timestamps, window_hours=24):
    ...
    aligned_rv = forward_rv.reindex(
        target_timestamps, method="nearest", tolerance=pd.Timedelta("5min")
    )
...
print(
    f"BTC Basis stats: Mean={btc_merged['basis'].mean():.4f}, Min={btc_merged['basis'].min():.4f}, Max={btc_merged['basis'].max():.4f}"
)
...
stats_btc_pre = get_summary_stats(
    pre_merge[pre_merge["asset"] == "BTC"], "BTC (Pre-Merge)"
)
stats_btc_post = get_summary_stats(
    post_merge[post_merge["asset"] == "BTC"], "BTC (Post-Merge)"
)
stats_eth_pre = get_summary_stats(
    pre_merge[pre_merge["asset"] == "ETH"], "ETH (Pre-Merge)"
)
stats_eth_post = get_summary_stats(
    post_merge[post_merge["asset"] == "ETH"], "ETH (Post-Merge)"
)

# Combine and transpose
table1 = pd.concat(
    [stats_btc_pre, stats_btc_post, stats_eth_pre, stats_eth_post], axis=1
).T
...
def _basis_log_series(df: pd.DataFrame, freq: str = "1D") -> pd.Series:
    ...
    resampled = df.set_index("datetime")["basis"].resample(freq).mean().dropna()
...
def compute_bsadf_path(
    ...
    for end in range(min_window, len(log_series) + 1, step):
        ...
        result = get_bsadf_statistic(
            log_price=window_series, min_sample_length=min_window, constant="ct", lags=1
        )
...
def _build_meta_dataset(
    ...
    raw_events = rl_labeling.symmetric_cusum_filter(
        basis_series, threshold=cusum_thresh
    )
...
model = RandomForestClassifier(
    n_estimators=200, max_depth=5, class_weight="balanced_subsample", random_state=42
)
model.fit(
    X, y
)  # Fit once to get baseline (Simulated for brevity, full PurgedCV recommended in production)
...
if "X" in locals() and "cluster_config" in locals():
    ...
    print(
        "⚠️ Variables X or cluster_config not found. Run the Meta-Labeling cell first."
    )
```
...
def save_classification_performance(model, X, y, output_dir):
    ...
    report_path = os.path.join(
        os.path.dirname(output_dir), "tabs", "04_meta_labeling_report.txt"
    )
...
def plot_parallel_trends(
    btc_data, eth_data, merge_date, window_days=90, save_path=None
):
    """
    ...
    ax.axvspan(
        ...
        color="green",
        alpha=0.05,
        label="Pre-Treatment (Parallel Trends)",
    )
    ax.axvspan(merge_date, end_date, color="red", alpha=0.05, label="Post-Treatment")

    ax.set_title(
        "Causal Inference: Parallel Trends Visualization",
        fontsize=16,
        fontweight="bold",
    )
