...
def plot_parallel_trends(
    ...
    trend_frame = trend_frame[
        (trend_frame.index >= start_date) & (trend_frame.index <= end_date)
    ]

    # Re-base both series to 100 at the window start for shape comparison.
    trend_frame["BTC_Indexed"] = (
        trend_frame["BTC_Vol"] / trend_frame["BTC_Vol"].iloc[0]
    ) * 100
    trend_frame["ETH_Indexed"] = (
        trend_frame["ETH_Vol"] / trend_frame["ETH_Vol"].iloc[0]
    ) * 100
