...
def create_continuous_futures(all_contracts_df, roll_days_before_expiry=14):
    ...
    for index, contract in enumerate(contracts):
        ...
        if index < len(contracts) - 1:
            next_contract_data = df[df['expiry'] == contracts[index + 1]].sort_values('datetime')
            ...
            segments[contract] = data[data['datetime'] <= roll_time].copy() if roll_time else data.copy()
        else:
    ...
    for index in range(len(contracts) - 1, -1, -1):
        ...
        if index == len(contracts) - 1:
            ...
            if not current_close_slice.empty and not next_close_slice.empty:
                ...
                adjustment = 0
                print(f"Warning: Gap detected at roll for {contract}. Adjustment set to 0.")
    ...
    for contract in contracts:
        ...
        pieces.append(segment[['datetime', 'close', 'adjusted_price', 'adjustment', 'contract']])
...
def plot_continuous_futures_panel(
    ...
    for index, roll_date in enumerate(roll_dates):
        ...
        ax.axvline(roll_date, color='red', linestyle=':', linewidth=2.5, alpha=0.5, label=label)

    ax.set_title(title, fontsize=16, fontweight='bold', pad=15)
    ax.set_xlabel('Date', fontsize=13, fontweight='bold')
    ax.set_ylabel(ylabel, fontsize=13, fontweight='bold')
    ax.legend(loc='upper left', fontsize=10, ncol=4, frameon=True, shadow=True, fancybox=True)
    ax.grid(True, alpha=0.3, linestyle='--', linewidth=1)
    ax.tick_params(axis='both', which='major', labelsize=11)


def plot_adjustment_panel(ax, plot_df: pd.DataFrame, title: str, color: str, roll_dates: np.ndarray) -> None:
    """Plot cumulative backward adjustment amounts for one asset."""
    ax.step(plot_df['datetime'], plot_df['adjustment'], where='post', linewidth=3, color=color, alpha=0.8)
    ax.fill_between(plot_df['datetime'], 0, plot_df['adjustment'], step='post', alpha=0.3, color=color)
...
btc_plot = btc_continuous[
    (btc_continuous['datetime'] >= plot_start) & (btc_continuous['datetime'] <= plot_end)
]
eth_plot = eth_continuous[
    (eth_continuous['datetime'] >= plot_start) & (eth_continuous['datetime'] <= plot_end)
]
