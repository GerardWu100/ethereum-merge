...
def build_meta_dataset(
    ...
    raw_events = rl_labeling.symmetric_cusum_filter(basis_series, threshold=cusum_threshold)
...
def plot_cluster_heatmap(feature_frame: pd.DataFrame, cluster_dict: dict[str, list[str]]) -> None:
...
def save_classification_performance(model, feature_frame, labels, output_dir: str) -> None:
    ...
    display_obj = ConfusionMatrixDisplay(confusion_matrix=confusion, display_labels=model.classes_)
...
cluster_config = {f'Cluster_{index}': members for index, members in clusters_dict.items()}
...
baseline_loss = log_loss(labels, model.predict_proba(feature_frame), labels=model.classes_)
...
for cluster_name, features in cluster_config.items():
    ...
    shuffled_loss = log_loss(labels, model.predict_proba(shuffled_frame), labels=model.classes_)
    importances[cluster_name] = shuffled_loss - baseline_loss

importance_frame = pd.Series(importances).sort_values(ascending=False).to_frame('Mean Decrease Accuracy')
...
importance_table = importance_frame.rename(columns={'Mean Decrease Accuracy': 'Importance (MDA)'})
