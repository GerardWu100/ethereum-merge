...
def apply_smoke_window(
    ...
    return df.loc[
        (df[datetime_col] >= smoke_start) & (df[datetime_col] <= smoke_end)
    ].copy()
...
btc_futures_pattern = os.path.join(
    CRYPTO_DIR, "DA-16_BTC_USDT_*_binance_quarterly.parquet"
)
eth_futures_pattern = os.path.join(
    CRYPTO_DIR, "DA-16_ETH_USDT_*_binance_quarterly.parquet"
)
